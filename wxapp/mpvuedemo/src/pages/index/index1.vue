<template>
  <div class="mpvue-demo">
    ...
    <button @click="addTodo">加一条<tton>
    <button @click="clearTodo">清空<tton>
    <ul class="todos">
      <!-- v-for v-key   vue的循环指令 用in指定  key + index
      : 动态绑定了属性-标签或组件的属性是跟data相关的 -->
      <li v-key='i' v-for="(todo,i) in todos" @click="toggle(i)" :class="{'done': todo.done}">{{todo.text}}<>
      <li>{{todoNum}}/{{todos.length}}<>
    </ul>
  </div>
</template>
...
<template>
  ...
  export default {
    ...
    components: {
      computed:{
        todoNum(){
          return this.todos.filter(todo=>!todos.done).length
        }
      }
    },
    methods: {
